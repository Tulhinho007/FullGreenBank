...
import { X, Plus, Pencil, Trash2, Dumbbell, Search, AlertTriangle } from 'lucide-react'
...
const EMOJI_RULES: [RegExp, string][] = [
  [/basqu|basket/i,                 '🏀'],
  [/futeb.*amer|american|nfl/i,     '🏈'],
  [/futebol|soccer|fut\b/i,         '⚽'],
  [/t[eê]nis.*mesa|ping.?pong/i,    '🏓'],
  [/t[eê]nis|tennis/i,              '🎾'],
  [/v[oô]lei|volley/i,              '🏐'],
  [/mma|boxe|box\b|muay|luta/i,     '🥊'],
  [/esport|games?|gaming/i,         '🎮'],
  [/cavalo|hipismo|eques/i,         '🐎'],
  [/auto.*mob|f[- ]?1|formula|nascar/i, '🏎️'],
  [/beiseb|baseball/i,              '⚾'],
  [/golfe?|golf/i,                  '⛳'],
  [/rugby/i,                        '🏉'],
  [/nata[cç]|swim/i,                '🏊'],
  [/ciclis|bike|biciclet/i,         '🚴'],
  [/atletis/i,                      '🏃'],
  [/h[oó]quei|hockey/i,             '🏒'],
  [/handeb?ol|handball/i,           '🤾'],
  [/sinuca|bilhar|snooker/i,        '🎱'],
  [/dardo|darts/i,                  '🎯'],
  [/cr[ií]quet|cricket/i,           '🏏'],
  [/skate/i,                        '🛹'],
  [/surf/i,                         '🏄'],
  [/jud[oô]|karate|carate/i,        '🥋'],
  [/arco.*flecha|arch/i,            '🏹'],
  [/remo\b|rowing/i,                '🚣'],
]

const getEmoji = (name: string): string => {
  for (const [p, e] of EMOJI_RULES) if (p.test(name)) return e
  return '🏅'
}
...
// ── SportSearchInput ──────────────────────────────────────────────────────────
const SportSearchInput = ({ sports, onSelect }: { sports: Sport[]; onSelect: (s: Sport) => void }) => {
  ...
  const pick = (s: Sport) => { setSelected(s); setQ(s.name); onSelect(s) }
...
export const SportsModal = ({ isOpen, onClose, sports, onSave, readOnly }: SportModalProps) => {
  ...
  const [delTarget,  setDelTarget]  = useState<Sport | null>(null)
  ...
  useEffect(() => {
    if (isOpen && actionMode === 'add') setTimeout(() => addRef.current?.focus(), 100)
  }, [isOpen, actionMode])

  if (!isOpen) return null

  const closeAction = () => {
    setActionMode(null); setAddName(''); setEditTarget(null); setEditName(''); setDelTarget(null)
  }
  ...
  const handleAdd = () => {
    ...
    setConfirm({
      title: 'Adicionar esporte', variant: 'success',
      message: `Adicionar "${addName.trim()}" ${getEmoji(addName.trim())} à lista?`,
      label: '✓ Adicionar',
      fn: () => {
        onSave([...sports, { id: crypto.randomUUID(), name: addName.trim(), emoji: getEmoji(addName.trim()), slug: makeSlug(addName.trim()) }])
  ...
  const handleEdit = () => {
    ...
    setConfirm({
      ...
      message: `Alterar "${editTarget.name}" para "${editName.trim()}" ${getEmoji(editName.trim())}?`,
      label: 'Salvar',
      fn: () => {
        onSave(sports.map(s => s.id === editTarget.id ? { ...s, name: editName.trim(), emoji: getEmoji(editName.trim()), slug: makeSlug(editName.trim()) } : s))
        closeAction(); setConfirm(null)
      }
    })
  }
  ...
  const handleDelete = () => {
    if (!delTarget) return
    setConfirm({
      title: 'Remover esporte', variant: 'danger',
      message: `Remover "${delTarget.name}" ${delTarget.emoji} permanentemente?`,
      label: 'Remover',
      fn: () => {
        onSave(sports.filter(s => s.id !== delTarget.id))
        closeAction(); setConfirm(null)
      }
    })
  }
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div className="w-full max-w-md pointer-events-auto bg-white rounded-2xl border border-slate-200  flex flex-col"
