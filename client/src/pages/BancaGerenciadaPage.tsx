...
const emptyForm = {
  ...
  status: 'ATIVO' as ContractStatus, motivoFim: '' as MotivoFim, observacoes: '',
}
...
export const BancaGerenciadaPage = () => {
  ...
  const filtered = contracts.filter(c => {
    ...
    return (!t || c.userName.toLowerCase().includes(t) || c.userEmail.toLowerCase().includes(t)) && (!filterStatus || c.status === filterStatus)
  })

  const setA = (f: string) => (e: React.ChangeEvent<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>) => setAddForm(p => ({ ...p, [f]: e.target.value }))
  const setE = (f: string) => (e: React.ChangeEvent<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>) => setEditForm(p => ({ ...p, [f]: e.target.value }))
  ...
  const handleAdd = async (e: FormEvent) => {
    ...
    } catch { toast.error('Erro ao criar contrato.') } finally { setSaving(false) }
  }

  const openEdit = (c: BancaContract) => {
    setEditTarget(c)
    setEditForm({ userId: c.userId, dataInicial: c.dataInicial.split('T')[0], dataFinal: c.dataFinal ? c.dataFinal.split('T')[0] : '',
      bancaInicial: String(c.bancaInicial), bancaFinal: String(c.bancaFinal), comissaoPercent: String(c.comissaoPercent),
  ...
  const handleRenew = async () => {
    ...
    } catch { toast.error('Erro ao renovar.') } finally { setSaving(false) }
  }

  // Preview de cálculos
  const Preview = ({ bi, bf, cp, di, df }: { bi: string; bf: string; cp: string; di: string; df: string }) => {
    const bancaI = Number(bi), bancaF = Number(bf || bi), com = Number(cp)
    ...
    return (
      <div className="bg-surface-300/60 border border-surface-400 rounded-lg px-4 py-3">
        ...
        <div className="grid grid-cols-4 gap-3 text-xs">
          <div><p className="text-slate-500 mb-0.5">Lucro</p><p className={`font-bold font-mono text-sm ${lucro > 0 ? 'text-green-400' : 'text-slate-500'}`}>{lucro > 0 ? '+' : ''}{fmt(lucro)}</p></div>
          <div><p className="text-slate-500 mb-0.5">Vl. Comissão</p><p className="text-yellow-400 font-bold font-mono text-sm">{fmt(vlCom)}</p></div>
          <div><p className="text-slate-500 mb-0.5">Vl. Cliente</p><p className="text-white font-bold font-mono text-sm">{fmt(vlCli)}</p></div>
          <div><p className="text-slate-500 mb-0.5">Duração</p><p className="text-slate-300 font-bold text-sm">{dias ? `${dias} dias` : '—'}</p></div>
        </div>
      </div>
    )
  }
  ...
  // Form reutilizável
  const ContractForm = ({ form, set, isEdit = false }: {
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <select className="input-field" value={form.userId} onChange={set('userId')} required>
          {!isEdit && <option value="">— Selecione o usuário —</option>}
          {users.map(u => <option key={u.id} value={u.id}>{u.email} ({u.email})</option>)}
        </select>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div><label className="label">Data Inicial *</label><input type="date" className="input-field" value={form.dataInicial} onChange={set('dataInicial')} required /></div>
        <div><label className="label">Data Final</label><input type="date" className="input-field" value={form.dataFinal} onChange={set('dataFinal')} /></div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div><label className="label">Banca Inicial (R$) *</label><input type="number" min="0.01" step="0.01" className="input-field" placeholder="Ex: 1000.00" value={form.bancaInicial} onChange={set('bancaInicial')} required /></div>
        <div><label className="label">Banca Final (R$)</label><input type="number" min="0" step="0.01" className="input-field" placeholder="Ex: 1500.00" value={form.bancaFinal} onChange={set('bancaFinal')} /></div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div><label className="label">Comissão (%) *</label><input type="number" min="0" max="100" step="0.1" className="input-field" value={form.comissaoPercent} onChange={set('comissaoPercent')} required /></div>
        <div>
          <label className="label">Status *</label>
          <select className="input-field" value={form.status} onChange={set('status')}>
            {(Object.keys(STATUS_CONFIG) as ContractStatus[]).map(s => <option key={s} value={s}>{STATUS_CONFIG[s].label}</option>)}
          </select>
        </div>
      </div>
      <div>
        <label className="label">Motivo do Fim</label>
        <select className="input-field" value={form.motivoFim} onChange={set('motivoFim')}>
          {(Object.keys(MOTIVO_LABEL) as MotivoFim[]).map(m => <option key={m} value={m}>{MOTIVO_LABEL[m]}</option>)}
        </select>
      </div>
      <div><label className="label">Observações</label><textarea className="input-field resize-none" rows={3} placeholder="Anotações sobre o contrato..." value={form.observacoes} onChange={set('observacoes')} /></div>
      <Preview bi={form.bancaInicial} bf={form.bancaFinal} cp={form.comissaoPercent} di={form.dataInicial} df={form.dataFinal} />
    </div>
  )
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-2"><div className="w-7 h-7 rounded-lg bg-green-900/50 flex items-center justify-center"><Briefcase size={13} className="text-green-400" /></div><span className="text-xs text-slate-500 uppercase tracking-wide">Contratos Ativos</span></div>
        ...
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-2"><div className="w-7 h-7 rounded-lg bg-green-900/50 flex items-center justify-center"><DollarSign size={13} className="text-green-400" /></div><span className="text-xs text-slate-500 uppercase tracking-wide">Banca Total Ativa</span></div>
        ...
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-2"><div className="w-7 h-7 rounded-lg bg-yellow-900/40 flex items-center justify-center"><Percent size={13} className="text-yellow-400" /></div><span className="text-xs text-slate-500 uppercase tracking-wide">Comissão Acumulada</span></div>
        ...
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-2"><div className="w-7 h-7 rounded-lg bg-green-900/50 flex items-center justify-center"><TrendingUp size={13} className="text-green-400" /></div><span className="text-xs text-slate-500 uppercase tracking-wide">Lucro Plataforma</span></div>
        ...
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-2"><div className="w-7 h-7 rounded-lg bg-blue-900/40 flex items-center justify-center"><Users size={13} className="text-blue-400" /></div><span className="text-xs text-slate-500 uppercase tracking-wide">Lucro Clientes</span></div>
          <p className={`text-2xl font-bold ${lucroClientes >= 0 ? 'text-blue-400' : 'text-red-400'}`}>{loading ? '—' : fmt(Math.abs(lucroClientes))}</p>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input className="input-field pl-9 py-2 text-sm" placeholder="Buscar por nome ou e-mail..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        <select className="input-field py-2 pr-8 text-sm appearance-none cursor-pointer" value={filterStatus} onChange={e => setFilterStatus(e.target.value as ContractStatus | '')}>
      ...
      {/* Tabela */}
      {loading ? (
        <div className="flex items-center justify-center py-16"><div className="w-8 h-8 border-2 border-green-500 border-t-transparent rounded-full animate-spin" /></div>
      ) : (
        <div className="card overflow-hidden">
          <div className="hidden xl:grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr_120px] gap-3 px-5 py-3 border-b border-surface-300 text-xs text-slate-500 uppercase tracking-wide">
            <span>Usuário</span><span>Período</span><span>B. Inicial</span><span>B. Final</span><span>Lucro</span><span>Comissão</span><span>Status</span><span className="text-right">Ações</span>
          </div>
          {filtered.length === 0 ? (
      ...
      <Modal isOpen={addOpen} onClose={() => { setAddOpen(false); setAddForm(emptyForm) }} title="Novo Contrato de Banca" size="md">
        <form onSubmit={handleAdd} className="flex flex-col gap-0">
          <ContractForm form={addForm} set={setA} />
      ...
      <Modal isOpen={!!editTarget} onClose={() => setEditTarget(null)} title="Editar Contrato" size="md">
        {editTarget && (
          <form onSubmit={handleEdit} className="flex flex-col gap-0">
            <ContractForm form={editForm} set={setE} isEdit />
            <div className="flex gap-3 pt-4 mt-2">
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-red-900/20 border border-red-800/40 rounded-lg px-4 py-3 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Contrato de <strong className="text-white">{deleteTarget.userName}</strong> · <strong className="text-white">{fmt(deleteTarget.bancaFinal)}</strong></p>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleDelete} disabled={saving} className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2">
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-surface-300/60 border border-surface-400 rounded-lg px-4 py-3">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
                <div><p className="text-slate-500">Novo Valor Inicial</p><p className="text-green-400 font-bold font-mono text-sm">{fmt(renewTarget.vlCliente > 0 ? renewTarget.vlCliente : renewTarget.bancaInicial)}</p></div>
                <div><p className="text-slate-500">Comissão mantida</p><p className="text-yellow-400 font-bold text-sm">{renewTarget.comissaoPercent}%</p></div>
              </div>
            </div>
            <div className="flex gap-3">
...
interface RowProps { contract: BancaContract; onEdit: (c: BancaContract) => void; onDelete: (c: BancaContract) => void; onRenew: (c: BancaContract) => void }
  ...
  return (
    <div className="px-5 py-4 border-b border-surface-300 last:border-0 hover:bg-surface-300/30 transition-colors group">
      <div className="flex flex-col xl:grid xl:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr_120px] gap-2 xl:gap-3 xl:items-center">
        ...
        <span className={`text-sm font-mono font-medium ${c.lucro > 0 ? 'text-green-400' : 'text-slate-500'}`}>{c.lucro > 0 ? '+' : ''}{fmt(c.lucro)}</span>
        <span className="text-sm text-yellow-400 font-mono">{fmt(c.vlComissao)}<span className="text-slate-600 text-xs ml-1">({c.comissaoPercent}%)</span></span>
        <div>
          <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border ${st.color}`}>{st.icon}{st.label}</span>
        ...
        <div className="flex items-center gap-1.5 xl:justify-end">
          {encerrado ? (
            <button onClick={() => onRenew(c)} className="flex items-center gap-1.5 text-xs text-green-400 border border-green-800/50 bg-green-900/30 px-2.5 py-1.5 rounded hover:bg-green-800/50 transition-colors">
              <RefreshCw size={11} />Renovar
            </button>
          ) : (
            <>
              <button onClick={() => onEdit(c)} title="Editar" className="flex items-center gap-1.5 text-xs text-slate-300 border border-surface-400 bg-surface-300 px-2.5 py-1.5 rounded hover:border-green-700/50 hover:text-green-400 transition-colors">
                <Edit2 size={11} /><span className="hidden sm:inline">Editar</span>
              </button>
              <button onClick={() => onDelete(c)} title="Excluir" className="flex items-center gap-1.5 text-xs text-slate-400 border border-surface-400 bg-surface-300 px-2.5 py-1.5 rounded hover:border-red-700/50 hover:text-red-400 transition-colors">
